...
def Get_Prep_Annotation(imgDir,JsonPath):
    ...
    # Проверьте, что файл имеет расширение изображения (например, .jpg, .png и т. д.)
        if filename.endswith(('.jpg', '.jpeg', '.JPG', '.gif', '.bmp')):
        # Если файл является изображением, добавьте его имя в список
            images_file.append(os.path.basename(filename))
    # print(images_file)
    ...
    for TargetImg in images_file:
        ...
        for Annotation in Annotation_Json['annotations']:
            if Annotation['image_id'] == Target_ID:
                ...
                if bbox[0]>img_in_RGB.shape[1]: 
                    
                    bbox[0] = img_in_RGB.shape[1]
                    print("bbox[0]>img_width")
                    print(TargetImg)
                if bbox[1]>img_in_RGB.shape[0]: 
                    bbox[1] = img_in_RGB.shape[0]
                    print("bbox[1]>img_height")
                    print(TargetImg)
                ...
                if bbox[1]<0:
                    ...
                    print(TargetImg)
                    bbox[1]=0
                if bbox[0]+bbox[2]/2>img_in_RGB.shape[1]: 
                    bbox[3] = img_in_RGB.shape[1] -  bbox[0]
                    print("bbox[2]")
                    print(TargetImg)

                if bbox[1]+bbox[3]/2>img_in_RGB.shape[0]: 
                    bbox[2] = img_in_RGB.shape[0] -  bbox[1]
